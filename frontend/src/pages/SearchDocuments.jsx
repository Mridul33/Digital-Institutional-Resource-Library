...
// Helper function to convert coverType to proper MIME type
const getMimeType = (coverType) => {
  if (!coverType) return "image/jpeg"; // default fallback
  ...
  const lowerType = coverType.toLowerCase().trim();
  
  // If it already contains a "/" (like "image/jpeg"), return as is
  if (lowerType.includes("/")) {
    return lowerType;
  }
  
  // Otherwise map it to proper MIME type
  return mimeMap[lowerType] || "image/jpeg";
};
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-6 mb-4 relative flex flex-col items-center">
      ...
      <input
        ...
        onKeyDown={(e) => { if (e.key === 'Enter') searchDocument(); }}
      />
        ...
        <Modal onClose={closeModal}>
          <div className="flex flex-col items-center p-6">
            ...
            viewingDocument?.coverType.includes("pdf") ? (
              ...
              <img
                src={`data:${getMimeType(viewingDocument?.coverType)};base64,${viewingDocument?.cover}`}
